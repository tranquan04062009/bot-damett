...
# Token cố định thay vì nhập tay
tok = "7446145238:AAE272hDRYFx6hWka_BF5AkV5IPGbA7b5bY"  # Thay YOUR_BOT_TOKEN_HERE bằng token thực tế của bạn
zzk = 0
...
def st(call):
    if call.data == "zn":
        ...
        MC = bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='Gửi số lượng báo cáo cần thực hiện', reply_markup=nc1)
...
def ass(message, sufi):
    ...
    add = int(sufi)
    bot.send_message(message.chat.id, f"<strong>Đang gửi báo cáo, vui lòng đợi...</strong>", parse_mode="html", reply_markup=types.InlineKeyboardMarkup())
    for i in range(add):
        addd += 1
        cookies = {
        ...
        if "status_message" or "status_code" or "extra" in re:
        ...
        if int(addd) == int(add):
            bot.send_message(message.chat.id, f"<strong>Đã hoàn thành số lượng báo cáo yêu cầu thành công ✅</strong>", parse_mode="html", reply_markup=types.InlineKeyboardMarkup())
